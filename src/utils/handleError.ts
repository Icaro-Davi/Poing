...
const handleError = (error: any, options: HandleErrorOptions) => {
    ...
    options.message.reply(options.customMessage || options.locale.error[error.code as LocaleErroTypes || 'unknown']).catch(reason => console.log('[error on handleError] - Error on send message', reason));
}
